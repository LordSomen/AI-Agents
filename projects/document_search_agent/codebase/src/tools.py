...
def create_calculator_tool(logger: ToolLogger):
    ...
    # Create a safe dictionary of math functions (sqrt, sin, log, pi, etc.)
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: The math expression to evaluate. 
                       ...
                       standard math functions (sqrt, log, sin, cos, pi, etc).
                       Do not use the 'math.' prefix. Example: 'sqrt(16) + 2^3'
        """
        try:
            # Phase 2: Convert ^ to ** for Python
            clean_expr = expression.replace(" ", "")
            clean_expr = clean_expr.replace("^", "**")
            
            # Security: Only allow letters, numbers, basic operators, and parentheses.
            # This blocks quotes, brackets, and equals signs to prevent code injection.
            if not re.match(r'^[a-zA-Z0-9+\-*/.(),]+$', clean_expr):
                return "Error: Expression contains invalid or dangerous characters."
                
            # Safely evaluate! 
            # {"__builtins__": {}} completely disables dangerous Python functions like __import__
            # SAFE_MATH provides our approved math functions
...
class AmountExtraction(BaseModel):
    has_amount_logic: bool = Field(description="True if the query mentions money, amounts, or financial conditions")
    comparison: Optional[Literal["over", "under", "between", "exact", "approximate"]] = None
    amount: Optional[float] = None
    min_amount: Optional[float] = None
    max_amount: Optional[float] = None

def create_document_search_tool(retriever, logger: ToolLogger, llm: Optional[Any] = None):
    """
    ...
    def document_search(
        ...
        try:
            ...
            if search_type == "keyword":
                ...
                # PHASE 3: NLP LLM Extraction
                extraction = None
                if llm:
                    try:
                        # Use the LLM to intelligently extract the numbers from the messy text!
                        extractor = llm.with_structured_output(AmountExtraction)
                        extraction = extractor.invoke(f"Extract financial conditions from this search query: {query}")
                    except Exception as e:
                        pass
                
                # If the LLM successfully found amounts, use them!
                if extraction and extraction.has_amount_logic:
                    results = _handle_amount_search(
                        retriever, 
                    ...
                    # THE FALLBACK: The existing brittle string checking
                    ...
                    if any(word in query_lower for word in ['over', 'under', 'above', 'below', 'between', 'around', 'exactly', '$']):
                        ...
                        for d_type in ['invoice', 'contract', 'claim']:
                            if d_type in query_lower:
                                results = retriever.retrieve_by_type(d_type)
                                break
                    else:
...
def get_all_tools(retriever, logger: ToolLogger, llm: Optional[Any] = None) -> List:
    """
    Get all available tools for the agent.
    """
    return [
        create_calculator_tool(logger),
        create_document_search_tool(retriever, logger, llm),
